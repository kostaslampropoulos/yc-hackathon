...
import { accrueCallCost } from "@/lib/billing";
...
async function handleCallEnded(data: CallEndedData, timer: StepTimer): Promise<void> {
  ...
  // Accrue Sponge per-call cost on the business's pending bill.
  const effectiveDuration =
    typeof durationSeconds === "number" && durationSeconds > 0
      ? durationSeconds
      : Math.max(
          1,
          Math.round((endedAt.getTime() - new Date(conversation.startedAt).getTime()) / 1000),
        );
  await accrueCallCost(conversation.businessId, effectiveDuration).catch((err) => {
    console.warn(`[webhook] accrueCallCost failed:`, err);
  });
  timer.step("billing.accrue");
